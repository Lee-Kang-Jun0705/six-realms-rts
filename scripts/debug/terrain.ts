...
mkdirSync('test-results/terrain',{recursive:true});
const run=async()=>{
  ...
  await page.locator('.menu-start.spec').click(); // 관전 = 안개 적어 지형 잘 보임
  await page.locator('.hud-res').waitFor({timeout:20000});
  await page.mouse.click(640,400);
  await page.waitForTimeout(4000);
  await page.screenshot({path:'test-results/terrain/terrain.png'});
