...
function runOne(seed: number): { winner: number; ticks: number; ms: number; violations: InvariantViolation[]; crash: string | null; produced: number } {
  const t0 = Date.now();
  ...
  return { winner: state.winner, ticks: state.tick, ms: Date.now() - t0, violations, crash, produced };
}
...
for (let s = 1; s <= SEEDS; s++) {
  ...
  if (ok) pass++;
  console.log(`  시드 ${s}: ${team} | ${r.ticks}틱 ${r.ms}ms | 생산 ${r.produced} | 불변식 ${r.violations.length} | ${r.crash ? '크래시: ' + r.crash.split('\n')[0] : 'OK'}`);
